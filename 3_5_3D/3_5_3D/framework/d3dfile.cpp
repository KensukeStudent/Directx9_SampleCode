...
// Desc:
//-----------------------------------------------------------------------------
CD3DMesh::CD3DMesh(const TCHAR* strName)
{
    wcsncpy_s(m_strName, strName, sizeof(m_strName) / sizeof(TCHAR));
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CD3DMesh::Create(LPDIRECT3DDEVICE9 pd3dDevice, LPD3DXFILEDATA  pFileData)
{
    LPD3DXBUFFER pMtrlBuffer = NULL;
    LPD3DXBUFFER pAdjacencyBuffer = NULL;
    HRESULT      hr;
    ...
    CD3DFrame* pParentFrame)
{
    ...
    if (guid == TID_D3DRMFrameTransformMatrix)
    {
        ...
        if (FAILED(hr = pFileData->Lock(&cbSize, &pData)))
            return hr;

        pmatMatrix = (D3DXMATRIX*)pData; // データをキャスト

        // Update the parent's matrix with the new one
        pParentFrame->SetMatrix(pmatMatrix);

        pFileData->Unlock(); // Unlockを忘れずに
    }
    ...
    if (guid == TID_D3DRMFrame)
    {
        ...
        SIZE_T dwNumChildren = 0;
        if (FAILED(hr = pFileData->GetChildren(&dwNumChildren)))
            return hr;
        ...
        for (DWORD i = 0; i < dwNumChildren; i++)
        {
            ...
            if (FAILED(hr = pFileData->GetChild(i, &pChildData)))
                return hr;

            pChildData->Release(); // 取得したデータは解放
            if (FAILED(hr = LoadFrame(pd3dDevice, pChildData, pCurrentFrame)))
                return hr;
        }
    }
...
HRESULT CD3DFile::CreateFromResource(LPDIRECT3DDEVICE9 pd3dDevice, TCHAR* strResource, TCHAR* strType)
{
    ...
    if (FAILED(hr = D3DXFileCreate(&pDXFile))) // 修正: DirectXFileCreate → D3DXFileCreate
        return E_FAIL;
    ...
    // 変更: トップレベルのオブジェクト数を取得
    SIZE_T dwNumTopLevelObjects = 0;
    hr = pEnumObj->GetChildren(&dwNumTopLevelObjects);
    if (FAILED(hr))
    {
        pEnumObj->Release();
        pDXFile->Release();
    ...
    // 変更: トップレベルのオブジェクトを取得して処理
    for (DWORD i = 0; i < dwNumTopLevelObjects; i++)
    {
        hr = pEnumObj->GetChild(i, &pFileData);
        if (FAILED(hr))
        {
            pEnumObj->Release();
            pDXFile->Release();
