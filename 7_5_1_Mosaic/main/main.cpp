...
HRESULT CMyD3DApplication::Render()
{
	...
	if (SUCCEEDED(m_pd3dDevice->BeginScene()))
	{
		...
		TSS(0, D3DTSS_COLOROP, D3DTOP_SELECTARG1); 	// ステージ0: 色は引数1（テクスチャ0＝m_pTex）の色をそのまま採用
		TSS(0, D3DTSS_COLORARG1, D3DTA_TEXTURE); 	// ステージ0 引数1 = テクスチャ0 の色
		TSS(1, D3DTSS_COLOROP, D3DTOP_SELECTARG2); 	// ステージ1: 色は引数2（テクスチャ1＝m_pSmallTex）を採用（ステージ0の色は使わない）
		TSS(1, D3DTSS_COLORARG1, D3DTA_CURRENT); 	// ステージ1 引数1 = 直前ステージの色（ここでは未使用だが明示）
		TSS(1, D3DTSS_COLORARG2, D3DTA_TEXTURE); 	// ステージ1 引数2 = テクスチャ1 の色（モザイク色）
		TSS(2, D3DTSS_COLOROP, D3DTOP_DISABLE); 	// ステージ2以降の色処理は無効化
		TSS(0, D3DTSS_ALPHAOP, D3DTOP_SELECTARG1); 	// ステージ0: アルファは引数1を採用（マスクのα）
		TSS(0, D3DTSS_ALPHAARG1, D3DTA_TEXTURE); 	// ステージ0 引数1 = テクスチャ0（m_pTex）のアルファ
		TSS(1, D3DTSS_ALPHAOP, D3DTOP_SELECTARG1); 	// ステージ1: アルファは引数1（現在のα＝マスクα）を保持
		TSS(1, D3DTSS_ALPHAARG1, D3DTA_CURRENT); 	// ステージ1 引数1 = 現在のアルファ
		TSS(2, D3DTSS_ALPHAOP, D3DTOP_DISABLE); 	// ステージ2以降のアルファ処理は無効化
		RS(D3DRS_ALPHABLENDENABLE, TRUE); 			// アルファブレンドを有効化
		RS(D3DRS_SRCBLEND, D3DBLEND_SRCALPHA);    	// 合成: ソース色の寄与は自身のα
		RS(D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA); 	// 合成: 背景色の寄与は (1 - ソースα)
		SAMP(0, D3DSAMP_MINFILTER, D3DTEXF_POINT); 	// サンプラ0 縮小フィルタ: ポイント（ニアレスト）
		SAMP(0, D3DSAMP_MAGFILTER, D3DTEXF_POINT); 	// サンプラ0 拡大フィルタ: ポイント（ニアレスト）
