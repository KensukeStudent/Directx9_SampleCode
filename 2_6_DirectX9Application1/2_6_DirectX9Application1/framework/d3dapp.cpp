...
HRESULT CD3DApplication::Create(HINSTANCE hInstance)
{
    ...
    if (m_hWnd == NULL)
    {
        ...
        WNDCLASS wndClass = { 0, WndProc, 0, 0, hInstance,
                              LoadIcon(hInstance, MAKEINTRESOURCE(IDI_MY26DIRECTX9APPLICATION1)),
...
HRESULT CD3DApplication::Initialize3DEnvironment()
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (pDeviceInfo->DevType == D3DDEVTYPE_HAL)
        {
            ...
            _tcsncat_s(m_strDeviceStats, _countof(m_strDeviceStats), szDescription, maxAppend);
        }
...
void CD3DApplication::UpdateStats()
{
    ...
    if (fTime - fLastTime > 1.0f)
    {
        ...
        {
			_sntprintf_s(strFmt, 100, _TRUNCATE, TEXT("backbuf %s, adapter %s"),
        ...
        if (m_d3dEnumeration.AppUsesDepthBuffer)
        {
            _sntprintf_s(strDepthFmt, 100, TEXT(" (%s)"),
        ...
        _sntprintf_s(m_strFrameStats, cchMaxFrameStats, _T("%.02f fps (%dx%d), %s%s%s"), m_fFPS,
