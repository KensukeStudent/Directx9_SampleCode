...
static const UINT MAP_SIZE = 1024;
...
CMyD3DApplication::CMyD3DApplication()
{
	...
	{
		m_pMeshs[i] = new CD3DMesh();
	}

	m_positions[0] = D3DXVECTOR3(-1.5f, 1, 1);
	m_positions[1] = D3DXVECTOR3(3, 1, 1);
	m_positions[2] = D3DXVECTOR3(1, 1, -2);
...
HRESULT CMyD3DApplication::InitDeviceObjects()
{
	...
	{
		// UFOモデル
		if (FAILED(hr = m_pMeshs[i]->Create(m_pd3dDevice, _T("ufo.x"))))
		{
			_com_error err(hr);
...
HRESULT CMyD3DApplication::RestoreDeviceObjects()
{
	...
	for (size_t i = 0; i < 3; i++)
	{
		m_pMeshs[i]->RestoreDeviceObjects(m_pd3dDevice);
	}
...
HRESULT CMyD3DApplication::Render()
{
	...
	if (SUCCEEDED(m_pd3dDevice->BeginScene()))	// 描画の開始
	{
		...
		// レンダリングターゲットの変更
		//-----------------------------------------------------
		m_pd3dDevice->SetRenderTarget(0, m_pShadowSurf); // --------------------シャドウマップのレンダーターゲットに切り替え
		...
		// オブジェクトの平均位置を計算してlookAtを決定
		D3DXVECTOR3 avgPosition = D3DXVECTOR3(0, 0, 0);
		for (size_t i = 0; i < 3; i++)
		{
			avgPosition += m_positions[i];
		}
		avgPosition /= 3.0f;
		...
		for (size_t i = 0; i < 3; i++)
		{
			D3DXVECTOR3 diff = m_positions[i] - avgPosition;
			float distance = D3DXVec3Length(&diff);
			if (distance > maxDistance)
				maxDistance = distance;
		}
		
		// 最低限の範囲を保証しつつ、オブジェクトの分布に応じて調整
		float orthoSize = max(15.0f, maxDistance * 2.0f + 5.0f);
		
		D3DXVECTOR3 lookAt = avgPosition; // オブジェクトの平均位置を中心に
		D3DXVECTOR3 up = D3DXVECTOR3(0, 1, 0);
		D3DXMatrixLookAtLH(&mV, &eye, &lookAt, &up);

		float orthoWidth = orthoSize;
		...
		for (size_t i = 0; i < 3; i++)
		{
			...
			m_pMeshs[i]->Render(m_pd3dDevice);
			m_pMeshs[i]->UseMeshMaterials(TRUE);
		}
...
HRESULT CMyD3DApplication::InvalidateDeviceObjects()
{
	...
	for (size_t i = 0; i < 3; i++)
	{
		m_pMeshs[i]->InvalidateDeviceObjects();
	}
