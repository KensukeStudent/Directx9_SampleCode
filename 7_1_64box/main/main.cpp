...
HRESULT CMyD3DApplication::RestoreDeviceObjects()
{
	...
	if (FAILED(m_pd3dDevice->CreateTexture(
		...
		D3DPOOL_DEFAULT, &m_pOriginalTex, NULL)))
		return E_FAIL;
...
HRESULT CMyD3DApplication::Render()
{
	...
	if (SUCCEEDED(m_pd3dDevice->BeginScene()))
	{
		...
		if (m_pEffect != NULL)
		{
			...
			m_pEffect->SetTexture(m_htSrcMap, m_pOriginalTex);
		...
		TVERTEX Vertex1[4] = {
			// x   y   z    rhw  tu tv
			{ w / 2, 0, 0.1f, 1.0f, 0.5, 0,},
			{   w, 0, 0.1f, 1.0f, 1.0, 0,},
			{   w, h, 0.1f, 1.0f, 1.0, 1,},
			{ w / 2, h, 0.1f, 1.0f, 0.5, 1,},
		};
...
#if 0 // デバッグ用にテクスチャを表示する
		{
			...
			for (DWORD i = 0; i < 2; i++) {
				...
				if (1 == i) m_pd3dDevice->SetTexture(0, m_pPostTex);
