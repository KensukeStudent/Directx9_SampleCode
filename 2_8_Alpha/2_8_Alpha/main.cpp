...
HRESULT CMyD3DApplication::InitDeviceObjects()
{
    ...
    if(FAILED(LoadTexture(
					 "earth.bmp"			// ファイル名
					, &m_pEarthTexture ) ) )// テクスチャオブジェクト
        return E_FAIL;

    // 雲テクスチャーを読み込む
    if( FAILED(LoadTexture( 
					  "cloud.bmp"			// ファイル名
					, &m_pCloudTexture ) ) )// テクスチャオブジェクト
        return E_FAIL;
...
HRESULT CMyD3DApplication::RestoreDeviceObjects()
{
    ...
    // 合成方法を設定する
	TSS( 0, D3DTSS_ALPHAOP,   D3DTOP_MODULATE);// 引数の成分を乗算する
    TSS( 0, D3DTSS_ALPHAARG1, D3DTA_TEXTURE );
    TSS( 0, D3DTSS_ALPHAARG2, D3DTA_DIFFUSE );

#define ALPHA_TYPE 0	// ここを変えると、合成方法が変わる
...
HRESULT CMyD3DApplication::Render()
{
    ...
    if( SUCCEEDED( m_pd3dDevice->BeginScene() ) )
    {
		...
		RS(D3DRS_ALPHABLENDENABLE, FALSE);// 半透明で描かない
		m_pd3dDevice->SetTexture( 0, m_pEarthTexture );
		m_pd3dDevice->DrawPrimitive( D3DPT_TRIANGLESTRIP, 0, 2 );
		
		RS(D3DRS_ALPHABLENDENABLE, TRUE);// 半透明で描画
...
{
    SAFE_RELEASE( m_pCloudTexture );	// テクスチャ
    SAFE_RELEASE( m_pEarthTexture );	// テクスチャ
