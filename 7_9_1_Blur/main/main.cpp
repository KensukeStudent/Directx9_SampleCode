...
HRESULT CMyD3DApplication::Render()
{
	...
	if (SUCCEEDED(m_pd3dDevice->BeginScene()))
	{
		const DWORD RENDERING_COUNT = 10;// 描画回数
		...
		for (i = 0; i < RENDERING_COUNT; i++) {
			...
			TSS(0, D3DTSS_COLOROP, D3DTOP_SELECTARG1); // テクスチャステージ0のカラー合成を「第1引数を選択（テクスチャ）」に設定
			TSS(0, D3DTSS_COLORARG1, D3DTA_TEXTURE);   // カラー引数1にテクスチャを指定

			RS(D3DRS_ALPHABLENDENABLE, TRUE);          // アルファブレンドを有効化（下地に重ね合わせる）
			RS(D3DRS_SRCBLEND, D3DBLEND_SRCALPHA);     // ソース（新規描画）のブレンドファクターにソースアルファ値を使用
			RS(D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA); // デスティネーション（背面）のブレンドファクターに(1−ソースアルファ)を使用
