...
HRESULT DXUtil_FindMediaFileCch(TCHAR* strDestPath, int cchDest, const TCHAR* strFilename)
{
    ...
    if (INVALID_HANDLE_VALUE != file)
    {
        wcsncpy_s(strDestPath, cchDest, strShortName, _TRUNCATE);
        strDestPath[cchDest - 1] = 0; // wcsncpy_s doesn't NULL term if it runs out of space
        CloseHandle(file);
        return S_OK;
    }
    ...
    wcsncpy_s(strDestPath, cchDest, strFilename, _TRUNCATE);
    strDestPath[cchDest - 1] = 0; // wcsncpy_s doesn't NULL term if it runs out of space
    ...
    DWORD cchDest, TCHAR* strDefault)
{
    ...
    if (ERROR_SUCCESS != RegQueryValueEx(hKey, strRegName, 0, &dwType,
        (BYTE*)strDest, &cbDest))
    {
        wcsncpy_s(strDest, cchDest, strDefault, _TRUNCATE);
        strDest[cchDest - 1] = 0;
...
#ifdef _UNICODE
    wcsncpy_s(wstrDestination, cchDestChar, tstrSource, _TRUNCATE);
...
#ifdef _UNICODE
    wcsncpy_s(tstrDestination, cchDestChar, wstrSource, _TRUNCATE);
...
VOID DXUtil_LaunchReadme(HWND hWnd, TCHAR* strLoc)
{
    ...
    if (strLastSlash)
    {
        wcsncpy_s(strExeName, &strLastSlash[1], MAX_PATH);
...
{
    int nResult = _snwprintf_s(strDest, cchDestChar, _TRUNCATE, TEXT("{%0.8X-%0.4X-%0.4X-%0.2X%0.2X-%0.2X%0.2X%0.2X%0.2X%0.2X%0.2X}"),
