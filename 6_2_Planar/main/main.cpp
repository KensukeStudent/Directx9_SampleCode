...
HRESULT CMyD3DApplication::InitDeviceObjects()
{
	...
	// 地面のテクスチャ
	D3DXCreateTextureFromFileEx(m_pd3dDevice, _T("tatami.bmp")
...
HRESULT CMyD3DApplication::FrameMove()
{
	...
	if (m_UserInput.bA && !m_UserInput.bD)
		m_ufoRot = wrapAngle(m_ufoRot + 0.01f, 0, 360);
	else if (m_UserInput.bD && !m_UserInput.bA)
		m_ufoRot = wrapAngle(m_ufoRot - 0.01f, 0, 360);

	// UFO の動き、(0.0, 0.7, 0.0)を中心に半径1.0でY軸周りの回転
	m_pos.x = (FLOAT)cos(m_ufoRot * 3.14f / 180);
	m_pos.z = (FLOAT)sin(m_ufoRot * 3.14f / 180);
	m_pos.y = 0.7f;
